...
const SHIFT_POINT = ProjectivePoint.fromAffinePoint(curve_params.SHIFT_POINT);
...
fn addPoints(acc: *ProjectivePoint, bits: []const bool, prep: []const AffinePoint) void {
    ...
    for (0..bits.len / CURVE_CONSTS_BITS) |i| {
        ...
        if (offset > 0) {
            ...
            acc.addAssignAffinePoint(prep[i * table_size + offset - 1]);
        }
    }
}
...
pub fn pedersenHash(x: Felt252, y: Felt252) Felt252 {
    ...
    addPoints(&acc, x_bits[0..248], CURVE_CONSTS_P0[0..]); // Add a_low * P1
    addPoints(&acc, x_bits[248..252], CURVE_CONSTS_P1[0..]); // Add a_high * P2
    addPoints(&acc, y_bits[0..248], CURVE_CONSTS_P2[0..]); // Add b_low * P3
...
test "pedersen-hash" {
    ...
    const hash = pedersenHash(in1, in2);
