...
pub const STARKNET_PRIME = @import("./constants.zig").STARKNET_PRIME;
...
test "Felt252 mulBy5" {
    ...
    try expectEqual(
        ...
        Felt252.fromInt(u256, std.math.maxInt(u256)).mulBy5().tou256(),
    );
}
...
test "Felt252 inv" {
    try expectEqual(
        ...
        Felt252.fromInt(u8, 10).inv().?.tou256(),
    );
    try expectEqual(
        @as(
            u256,
        ...
        Felt252.fromInt(u256, std.math.maxInt(u256)).inv().?.tou256(),
    );
    try expectEqual(
        @as(
            ?Felt252,
