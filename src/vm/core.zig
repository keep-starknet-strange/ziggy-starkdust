...
pub const CairoVM = struct {
    ...
    pub fn init(comptime capacity: usize, allocator: *const Allocator, config: Config) !CairoVM {
        ...
        if (config.enable_trace) {
            trace = try ArrayList(TraceEntry).initCapacity(allocator.*, capacity);
        }
...
test "update pc regular no imm" {
    ...
    var vm = try CairoVM.init(4096, &allocator, .{});
    defer vm.deinit();
...
test "trace is enabled" {
    ...
    var vm = try CairoVM.init(4096, &allocator, config);
    defer vm.deinit();
